...
class Summary():
    ...
    """
    def __init__(self, data=None, model=None, rel_position=True, n_clusters=16, lsa=False, mmr=False, 
    ...
    def clean_sent(self):
        ...
        for index, row in self.df.iterrows():
          if index % 10 == 0:
              logging.info("Cleaned " + index + "sentences")
          temp = util.clean_not_remain_tag(row["raw_text"])
          if len(temp) == 0:
            logging.info("Delete sentence" + index + "because of zero length")
            self.df.drop(labels=[index], inplace=True)
          else:
            self.df.at[index, "clean_text"] = temp
        logging.info("Finish cleaning sentences")
        logging.indo("Reindex ")
    ...
    def sent2vec(self):
        ...
        for index, row in self.df.iterrows():
            if index % 10 == 0:
              logging.info("Computed " + index + "vector sentences")
            self.df.at[index, "vector"] = util.compute_vector(self.model, row["raw_text"])
    ...
    def kmean_summary(self):
        ...
        self.summary.index = range(len(self.n_clusters))
    ...
    def MMR(self):
        if self.lsa:
            index = self.LSA()
            self.summary = self.summary.iloc[index]
        ...
        for i in range(15):
            ...
            for idx_v, row in self.summary[S1].iterrows():
                sim1 = util.sim(first_sent, idx_v)
                max_sim2 = 0
                if i > 0:
                    for idx_u, row_u in self.summary[S2].iterrows():
                        sim2 = util.sim(idx_u, idx_v)
                        max_sim2 = max(max_sim2, sim2)
                simV = self.lamda*(sim1 - (1-self.lamda)*sim2)
                if simV >= max_sim:
                    max_sim = simV
                    max_idx = idx_v
            self.summary[max_idx, 'choose'] = 2
